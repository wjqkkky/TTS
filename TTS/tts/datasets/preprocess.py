...
def private_data(root_path, meta_file):
	items = []
	with open(os.path.join(root_path, meta_file), "r", encoding="utf-8") as f:
		while 1:
			line = f.readline()
			...
			wav_file = os.path.join(root_path, "wavs", speaker_name, filename + ".wav")
			items.append([text, wav_file, 'private_' + speaker_name])
	return items
...
def niuman(root_path, meta_file):
	txt_file = os.path.join(root_path, meta_file)
	items = []
	speaker_name = "niuman"
	with open(txt_file, 'r') as ttf:
