...
# - to set file paths.
# - to download relalibritts model files from TTS.
# - download or clone relalibritts repos, linked below.
...
JSON_OUTPUT = "../../../../datasets/training/speaker_encoder/LibriTTS-common-voice-voxceleb_angle_proto/libriTTS-clean-100-and-360"
MODEL_PATH = MODEL_RUN_PATH + "320k.pth.tar"
CONFIG_PATH = MODEL_RUN_PATH + "config.json"
USE_CUDA = True

files =  glob('../../../../datasets/LibriTTS/LibriTTS/dataset-preprocessed/dataset-22k/**/*.wav', recursive=True)
#print(files[0])
meta_libritts = []
libritts_speakers = set()
for i in tqdm(range(len(files))):
    wav_file = files[i]
    speaker_id = os.path.basename(wav_file).split('_')[0]
    if speaker_id not in libritts_speakers:
        print(speaker_id)
    meta_libritts.append(['', wav_file, speaker_id])
    libritts_speakers.add(speaker_id)

meta_data= meta_libritts
print(" LibriTTS Speakers:", len(libritts_speakers))
# In[ ]:
...
print("The file speakers.json has been exporlibritts to ",JSON_OUTPUT, ' with ', len(embeddings_dict.keys()), ' speakers')
