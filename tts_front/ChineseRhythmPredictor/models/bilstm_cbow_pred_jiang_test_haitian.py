...
import time
import os
from tts_front.ChineseRhythmPredictor import parameter
...
class BiLSTM():
    ...
    def hierarchy(self,inputs,y_masked,seq_length,scope_name,reuse=False):
        ...
        with tf.variable_scope(name_or_scope=scope_name,reuse=reuse):
            ...
            # prediction
            # pred:[batch_size*max_time,]renjia
        ...
        with self.session as sess:
            ...
            for epoch in range(1, self.max_epoch + 1):
                ...
                if self.best_validation_loss < self.valid_loss:
                    ...
                    saver.save(sess, "./models/" + name + "/bilstm/my-model-5")
                    # Generates MetaGraphDef.
                    saver.export_meta_graph("./models/" + name + "/bilstm/my-model-5.meta")
                print("\n\n")

    def load_model(self):
        self.sess = tf.Session()
        # ckpt = tf.train.get_checkpoint_state('./ChineseRhythmPredictor/models/models/test/bilstm/my-model-5')
        new_saver = tf.train.import_meta_graph(
            meta_graph_or_file="./tts_front/ChineseRhythmPredictor/models/models/test/bilstm/my-model-5.meta",
            clear_devices=False
        )
        # clear_devices=True
        new_saver.restore(self.sess, "./tts_front/ChineseRhythmPredictor/models/models/test/bilstm/my-model-5")
        # get default graph
        # graph = tf.get_default_graph()
        graph = tf.get_default_graph()
        self.X_p = graph.get_operation_by_name("input_p").outputs[0]
        self.seq_len_p = graph.get_operation_by_name("seq_len").outputs[0]
        self.pos_p = graph.get_operation_by_name("pos_p").outputs[0]
        self.length_p = graph.get_operation_by_name("length_p").outputs[0]
        self.position_p = graph.get_operation_by_name("position_p").outputs[0]
        self.keep_prob_p = graph.get_operation_by_name("keep_prob_p").outputs[0]
        self.input_keep_prob_p = graph.get_operation_by_name("input_keep_prob_p").outputs[0]
        self.output_keep_prob_p = graph.get_operation_by_name("output_keep_prob_p").outputs[0]
        # pred_normal = graph.get_operation_by_name("pred_normal").outputs[0]
        self.pred_pw = graph.get_operation_by_name("pw/pred").outputs[0]
        self.pred_pph = graph.get_operation_by_name("pph/pred").outputs[0]
        return self.sess
    # def pred_jiang(self, X, len_test,pos_test,length_test,position_test):
    #     sess = self.sess
    #     pred_pw, pred_pph = sess.run(fetches=[pred_pw, pred_pph], feed_dict={X_p: X,
    #                                                                          seq_len_p: len_test,
    #                                                                          pos_p: pos_test,
    #                                                                          length_p: length_test,
    #                                                                          position_p: position_test,
    #                                                                          keep_prob_p: 1.0,
    #                                                                          input_keep_prob_p: 1.0,
    #                                                                          output_keep_prob_p: 1.0})
    #     return pred_pw, pred_pph

    def pred_rhy(self,ch):
        # print("Loading Data...")
        # model = BiLSTM()
        # ch = '你好，请问有什么可以帮助到你的吗。'
        X_test, pos_test, len_test, length_test, position_test = data_main(ch)
        # pred_pw, pred_pph = model.pred_jiang("test", X_test, len_test, pos_test, length_test, position_test)
        sess = self.sess
        pred_pw, pred_pph = sess.run(fetches=[self.pred_pw, self.pred_pph], feed_dict={self.X_p: X_test,
                                                                                       self.seq_len_p: len_test,
                                                                                       self.pos_p: pos_test,
                                                                                       self.length_p: length_test,
                                                                                       self.position_p: position_test,
                                                                                       ...
                                                                                       self.input_keep_prob_p: 1.0,
                                                                                       self.output_keep_prob_p: 1.0})
        # pred_pw_ndarray = np.reshape(pred_pw,(-1,60))
        # print(pred_pw_ndarray)
        # print(y_test[0])
        doc = util.recover2(
            X=X_test,
            preds_pw=pred_pw,
    ...
    def pred(self, name, X, len_test,pos_test,length_test,position_test,y=None):
        start_time = time.time()  # compute time
        if y is None:
            with self.session as sess:
                # restore model
                new_saver = tf.train.import_meta_graph(
                    meta_graph_or_file="./ChineseRhythmPredictor/models/models/" + name + "/bilstm/my-model-5.meta",
                    clear_devices=False
                )
                #clear_devices=True
                new_saver.restore(sess, "./ChineseRhythmPredictor/models/models/" + name + "/bilstm/my-model-5")
                # get default graph
                graph = tf.get_default_graph()
                # get opration from the graph
                # X_p = graph.get_operation_by_name("input_placeholder").outputs[0]
                X_p = graph.get_operation_by_name("input_p").outputs[0]
                seq_len_p = graph.get_operation_by_name("seq_len").outputs[0]
                pos_p = graph.get_operation_by_name("pos_p").outputs[0]
                length_p = graph.get_operation_by_name("length_p").outputs[0]
                position_p = graph.get_operation_by_name("position_p").outputs[0]
                keep_prob_p = graph.get_operation_by_name("keep_prob_p").outputs[0]
                input_keep_prob_p = graph.get_operation_by_name("input_keep_prob_p").outputs[0]
                output_keep_prob_p = graph.get_operation_by_name("output_keep_prob_p").outputs[0]
                # pred_normal = graph.get_operation_by_name("pred_normal").outputs[0]
                pred_pw = graph.get_operation_by_name("pw/pred").outputs[0]
                pred_pph = graph.get_operation_by_name("pph/pred").outputs[0]
                #print(pred_pw)
                pred_pw,pred_pph = sess.run(fetches=[pred_pw,pred_pph], feed_dict={X_p: X,
                    seq_len_p: len_test,
                    pos_p: pos_test,
            ...
            with self.session as sess:
                # restore model
                new_saver = tf.train.import_meta_graph(
                    meta_graph_or_file="./models/" + name + "/bilstm/my-model-5.meta",
                    clear_devices=True
                )
                new_saver.restore(sess, "./models/" + name + "/bilstm/my-model-5")
                graph = tf.get_default_graph()
                # get opration from the graph
...
def tag2id(chinese):
    ...
    for i in range(0, len(chinese)):
        if i == 0:
            ...
            for j in range(0, len(chinese[i])):
                if j == 0:
                    tag.append(int(chinese[i][0]))
                elif j == len(chinese[i]) - 1:
                    break
                else:
                    tag.append(0)
    return tag
...
#         preds_pw=pred_pw,
#         preds_pph=pred_pph,
#         filename="./ChineseRhythmPredictor/result/" + str(1111) + ".txt"
#     )
#     return doc
...
if __name__ == "__main__":
    ...
    # ch = '你好，请问有什么可以帮助到你的吗。'
    # X_test, pos_test, len_test, length_test, position_test = data_main(ch)
    # pred_pw, pred_pph = model.pred("test", X_test, len_test, pos_test, length_test, position_test)
    # # pred_pw_ndarray = np.reshape(pred_pw,(-1,60))
    # # print(pred_pw_ndarray)
    # # print(y_test[0])
    # doc = util.recover2(
    ...
    #     X_test, pos_test, len_test, length_test, position_test = data_main(ch)
    #     pred_pw, pred_pph = model.pred("test", X_test, len_test, pos_test, length_test, position_test)
    #     pred_pw_ndarray = np.reshape(pred_pw, (-1, 40))
    #     # print(pred_pw_ndarray)
    #     # print(y_test[0])
    #     doc = util.recover2(
    #         X=X_test,
    ...
    #     )
    #     end_tine = time.clock()
    #     print("花费时间：",end_tine-start_time)
    #     print(doc)
        ########
