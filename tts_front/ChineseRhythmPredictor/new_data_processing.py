...
def toWordEmbeddings(inFile):
    ...
    if os.path.exists("./data/embeddings/word_vec.txt"):
        ...
        for i in range(1, vocab_size + 1):
            ...
            ids.append(i)
        pd.DataFrame(data={"words": words, "id": ids}).to_csv(path_or_buf="./data/dataset/words_ids.csv", index=False, encoding="utf_8")
    else:
        print("there is no embedings files")
...
def toPW(inFile,outFile):
    ...
    lines = file.readlines()
    # 匹配#0标记,替换为/n
    pattern2 = re.compile(r"#0", flags=re.U)
    # pattern1 = re.compile(r"#[0,1,2,3,4]", flags=re.U)
    # 匹配#1 #2标记,替换为/b
...
def make_dataset(inFile,outFile):
    ...
    #数据集切分
    df_data_train=df_data[:-2000]
    df_data_valid=df_data[-2000:-1000]
    df_data_test = df_data[-1000:]
...
if __name__=="__main__":
    ...
    make_dataset(inFile="./data/corpus/prosody_iph.txt", outFile="iph")
