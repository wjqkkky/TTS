# -*- coding: UTF-8 -*-
from tts_front.change_tone import find_index
...
def is_reserved_symbols(uchar):
    '''
    保留需要的字符
    #{8220,“}；{8221,”}；{12289,，}；{12290,。}；{44,，}；{46,.};{59,;},{58,:},{33,!},{63,?}{42,*}；{32，空格}
    :param uchar:
    :return:
    '''
    inside_code = ord(uchar)
    list_symbols = [8220, 12289, 12290, 44, 46, 59, 58, 33, 63, 42]
...
def replace_first_rhy(mandarin, sig='“'):
    '''
    将特殊的符号转化为#1，
    思路：检查特殊符号sig前后是否为汉字，若前后均为汉字，则将sig转化为一级韵律符号#1
    :param mandarin: 转化前的字符串
    :return: 转化后的字符串
    '''
    indexs = find_index(mandarin, sig)
    new_mandarin = mandarin
    for index in indexs:
        flag_l, flag_r = 0, 0
        search_l, search_r = 1, 1
        i = 1
        while i:
            index_l, index_r = index - i, index + i
            i = i + 1
            if search_l:
                if index_l >= 0:
                    char = mandarin[index_l]
                    if char != '*':
                        char_uni = convert_toUnicode(char)
                        if is_chinese(char_uni):
                            flag_l = 1
                        else:
                            new_mandarin = new_mandarin.replace(sig, '', 1)
                            break
                        search_l = 0
                else:
                    new_mandarin = new_mandarin.replace(sig, '', 1)
                    ...
                    break
            if search_r:
                if index_r < len(mandarin):
                    char = mandarin[index_r]
                    if char != '*':
                        char_uni = convert_toUnicode(char)
                        if is_chinese(char_uni):
                            flag_r = 1
                    ...
                    if flag_l == 1:
                        new_mandarin = new_mandarin.replace(sig, '#1', 1)
                    else:
                        new_mandarin = new_mandarin.replace(sig, '', 1)
                    break
            if search_r == 0 and search_l == 0:
                if flag_r and flag_l:
                    new_mandarin = new_mandarin.replace(sig, '#1', 1)
                    break
    return new_mandarin
