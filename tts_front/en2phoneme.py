...
def trans2phone(list_final: list, chinese_split=True, chinese_u2v=True):
    """
    将单独的英文和中文转化为音素
    :param chinese_u2v: 是否将u转化为V
    :param chinese_split: 是否进行切分
    :param list_final:中英文分割后的列表
        ...
        if word[0].islower() or word[0].isupper():
            ...
            if word == ' ':
                ...
                for j in range(len(string_lists)):
                    ...
                    if first_flag:
                        rhy_num = string_lists_with_rhy_num[j][0]
                        string_list = string_lists_with_rhy_num[j][1:]
                    else:
                        string_list = string_lists_with_rhy_num[j]
                    if '儿' in string_list:
                        string_phone_temp = ch2py(string_list, u2v=False)
                        string_phone = ' '.join(
                            split_string_phone[index_now:index_now + len(string_phone_temp.split())])
...
def add_ch_en_segment_sgin(phone_final):
    ...
    for i in range(0, len(phone_final)):
        if flag:
            ...
            flag_now = words_flag(phone_final[i])  # 大写为0，小写为1，其他为2
            if flag_now == 2:
...
def word2phone(chinese:str, chinese_split=True, chinese_u2v=True):
    if chinese == '':
        return ''
    split_chinese_eng_list = split_chinese_eng(chinese)
